...
Subject: Chronicle of Philanthropy's Endowment Report Verification: PLEASE RESPOND BY TUESDAY, NOV. 5
...
If anything needs to be updated, please let us know immediately.  If we do not hear from you by Tuesday, November 5th, we will publish what you have already provided. Thank you for helping us ensure accuracy.
...
with open('Emails_endowments_1101.csv', 'rU') as csvfile:
	read = csv.reader(csvfile)
	counter = 0
	for x in read:
		#if not header row
		if counter != 0: 
			with open('Emails_endowments_1101.txt', 'a+') as textfile:
